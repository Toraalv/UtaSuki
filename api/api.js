	...
	jwt.verify(token, TOKEN_SECRET, async (e, data) => {
		if (e) req.authed = false;
			...
			if ((await dbQuery("SELECT 1 FROM users WHERE uid = ? AND auth_token = ?", [data.uid, token])).length) {
				req.authed = true;
				let user = await dbQuery("SELECT users.uid, username, created, image, image_ver, last_activity, public, notes_public, language, border_radius, body_margin, accent, accent_text, animations, bkg, bkg_ver, opacity FROM users NATURAL JOIN user_settings WHERE auth_token = ?;", [token]);
				req.profile = user[0];
			}
		}
		next();
